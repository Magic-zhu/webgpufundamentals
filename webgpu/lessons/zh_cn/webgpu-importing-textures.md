...
Description: 怎么样加载图像/画布/视频到纹理中
TOC: 加载图像

我们在[前面的文章]((webgpu-textures.html))中介绍了一些关于使用纹理的基础知识。
在本文中，我们将介绍将图像加载到纹理中以及在 GPU 上生成 mipmap。

在上一篇文章中，我们通过调用`device.createTexture`创建了一个纹理，然后通过调用`device.queue.writeTexture`
将数据放入纹理中。`device.queue`上还有另一个函数叫做`device.queue.copyExternalImageToTexture`，
它可以让我们将图像复制到纹理中。

它可以接收一个ImageBitmap，所以让我们采用[上一篇文章中的magFilter](webgpu-textures.html#a-mag-filter)示例并对其进行修改以加载一些图像。

首先，我们需要一些代码从图像中获取一个ImageBitmap。
...
上面的代码使用图像的 URL 调用`fetch`。这将返回一个`Response`。然后，我们使用它来加载一个`Blob`，
它表示不透明图像文件的数据。然后，我们将其传递给`createImageBitmap`，这是一个标准的浏览器函数，
用于创建一个`ImageBitmap`。我们传递{ colorSpaceConversion: 'none' }是为了告诉浏览器不要应用任何色彩空间。
是否让浏览器应用色彩空间由你决定。在 WebGPU 中，我们经常会加载法线贴图、高度贴图或其他非颜色数据的图像。
在这些情况下，我们绝对不希望浏览器篡改图像中的数据。

现在我们有了创建ImageBitmap的代码，让我们加载并创建一个相同大小的纹理。

我们将加载此图像。

<div class="webgpu_center"><img src="../resources/images/f-texture.png"></div>

我曾经被告知，带有F的纹理是一个很好的示例纹理，因为我们可以立即看到它的方向。
...
请注意，`copyExternalImageToTexture`要求我们包含`GPUTextureUsage.COPY_DST`和
`GPUTextureUsage.RENDER_ATTACHMENT`使用标志。

然后我们就可以将ImageBitmap复制到纹理中。
...
`copyExternalImageToTexture`的参数是源、目标和大小。
对于源，我们可以指定`flipY: true`，如果我们希望在加载纹理时进行翻转。
...
在[上一篇文章中，我们也生成了一个纹理映射（mipmap）](https://webgpufundamentals.org/webgpu/lessons/webgpu-textures.html#a-mipmap-filter)，但在那种情况下，我们很容易访问图像数据。在加载图像时，我们可以将该图像绘制到一个 2D 画布中，调用
`getImageData`获取数据，最后生成多级渐远纹理（mips）并上传。这会非常慢。
而且由于 2D 画布的渲染方式故意依赖于具体实现，所以这也可能会有损失。

当我们生成 MIP 层级时，我们进行了双线性插值，这与 GPU 对minFilter: linear的处理完全相同。
我们可以利用这个特性在 GPU 上生成 MIP 层级。

让我们修改上一篇文章中的mipmapFilter 示例，以使用 GPU 加载图像并生成 Mip 层级。

首先，让我们修改创建纹理的代码以创建 MIP 层级。我们需要知道要创建多少个，我们可以这样计算。
